...
impl NotificationAppService {
    ...
    pub async fn get_user_notifications(
        ...
        self.notification_repository.find_by_user(user_id, limit).await
    }
    ...
    pub async fn mark_as_read(&self, id: Uuid, user_id: Uuid) -> Result<(), DomainError> {
        ...
        let notification = self
            ...
            .find_by_id(id)
            .await?
            .ok_or_else(|| DomainError::NotFound("Notification not found".into()))?;

        if notification.user_id != user_id {
            return Err(DomainError::Forbidden("Not your notification".into()));
        }
